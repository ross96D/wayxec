...
class _StackOptionsListWidgetState<T extends Object> extends State<StackOptionsListWidget<T>> implements OptionsListRenderer {
  ...
  Widget build(BuildContext context) {
    ...
    for (int i=getRenderStart(); i<getRenderEnd(); i++) {
      ...
      _Item<T>? item;
      try { // would be prettier with dartx firstOrNullWhere
        item = items.firstWhere((item) => item.option==e);
      } catch (_) { }
      if (item!=null) {
        item.index = i;
      } else {
        items.add(_Item(index: i, option: e,));
      }
    }
